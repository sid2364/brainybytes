...
class BoltzmannJointActionLearner:
    ...
    def select_action_old(self, state):
        if np.random.rand() < self.epsilon:
            ...
            # Compute joint distribution as the outer product of individual distributions
            joint_distribution = np.outer(self.opponent1_action_distribution, self.opponent2_action_distribution)
            # print("ACTION SELECTION for ", self.name)
...
class BoltzmannJointActionLearner2:
    def __init__(
            self,
            action_size,
            opponent_action_size,
        ...
        self.opponent1_action_distribution = np.ones(self.opponent1_action_size) / self.opponent1_action_size
        self.opponent2_action_distribution = np.ones(self.opponent2_action_size) / self.opponent2_action_size
    ...
    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            ...
            # Compute expected Q-values assuming opponents act independently
            joint_distribution = np.outer(self.opponent1_action_distribution, self.opponent2_action_distribution)
            expected_q_values = np.sum(
                self.qtable[state] * joint_distribution,
                axis=(1, 2)
            ...
            exp_q_values = np.exp((expected_q_values - max_q_value) / self.temperature)
            action_probabilities = exp_q_values / np.sum(exp_q_values)
            print("action_probabilities", self.name, action_probabilities)
            action = np.random.choice(self.action_size, p=action_probabilities)
            print("action", action)
    ...
    def select_action3(self, state):
        if np.random.rand() < self.epsilon:
            ...
            # Step 1: Calculate the expected Q-values for your agent's actions
            expected_q_values = np.sum(
                self.qtable[state] * np.outer(self.opponent1_action_distribution, self.opponent2_action_distribution),
                axis=(1, 2)
            )
    ...
    def update_opponent_distribution3(self, opponent1_action, opponent2_action):
    ...
    def update3(self, state, action, opponent1_action, opponent2_action, new_state, reward, done, update_epsilon=True):
        if not self.dynamic_lr:
            lr = self.learning_rate
        else:
            # Dynamic learning rate for joint actions
